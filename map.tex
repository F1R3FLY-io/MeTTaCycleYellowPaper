...
\section{Appendix C: Inference rules generated by the hypercube functor acting on the GSLT for RHO calculus}
\label{Appendix C}
...
\BinaryInfC{$\Gamma \vdash A \mid C : s^P$}
\end{prooftree}
...
\begin{prooftree}
\AxiomC{$\Gamma \vdash D : \rm{for}(x, A, B, C)$}
\AxiomC{$\Gamma \vdash E : *B$}
\BinaryInfC{$\Gamma \vdash D \mid x!(E) : \Diamond (C\, E)$}
\end{prooftree}

\subsubsection*{(b) LHS Context}

\begin{prooftree}
\AxiomC{$\Gamma \vdash D : \rm{for}(x, A, B, \lambda y.C)$}
\AxiomC{$\Gamma \vdash E : *B$}
\BinaryInfC{$\Gamma \vdash D : \langle [\rm{for}(x, A, B, \lambda y.C)] \mid x!(E) \rangle (C\, E)$}
\end{prooftree}
...
\[
\rm{comm}:\quad x!(Q) \mid \rm{for}(x, @B, \lambda y.D) \leadsto \bigl((\lambda y.D)\; @Q\bigr).
\]
...
\AxiomC{$\Gamma,\, y:@B \vdash C : s_3^P$}
\AxiomC{$\Gamma,\, y:@B \vdash D : C$}
\SenaryInfC{$\Gamma \vdash x!(Q) \mid \rm{for}(x, @B, \lambda y.D) : \langle\rangle \bigl((\lambda y.C)\, @Q\bigr)$}
\end{prooftree}
...
The modality $\Diamond^*$ is defined as:
\[
\Diamond^* : P \to P.
\]
