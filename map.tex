...
hypothesis-free (or base) reduction rule of the form $K( P, E ) ~> K'(
...
\item $\lambda$-calculus:
  \begin{itemize}
  \item $K = App$, $P = Abs( [x]f )$, $E = arg$,
%%  \item $P' = f[ arg / x ]$, $E' = \empty$, $K' = \box$
  \item $App( Abs( [x]f ), arg ) ~> f[ arg / x ]$
  \end{itemize}
\item rho calculus:
  \begin{itemize}
    \item $K = Par$, $P = Recv( x, [y]R )$, $E = Send( x, Q )$,
%%    \item $P' = R[ @Q / y ]$, $E = 0$, $K' = \box$
    \item $Par( Recv( x, R ), Send( x, Q ) ) ~> R( @Q )$
  \end{itemize}
\item or
  \begin{itemize}
  \item $P = Send( x, Q )$, $E = Recv( x, R )$,
  \item $P' = 0, E' = R( @Q )$, etc
  \end{itemize}
\end{itemize}
...
F_{2_s}( E ) ) ~> F_t( F_{1_t}( P' ), F_{2_t}( E' ) )$ and $base( J ) = M_s(
M_{1_s}( P ), M_{2_s}( E ) ) ~> M_t( M_{1_t}( P' ), M_{2_t}( E' ) )$.  Then we
can define a collection of new theories, denoted $I \otimes J$. To
...
\begin{align}
  I \otimes J = \mathsf{for}( src \mathsf{<-} \mathsf{src}( I \otimes J ); trgt \mathsf{<-} \mathsf{trgt}( I \otimes J ) ) \mathsf{yield} \{ \\
  \mathsf{Theory} \mathsf{mkName}( src, trgt )() \{ \\
       \mathsf{Nil} \\
    \mathsf{Terms} \{ \mathsf{Terms}( I ) \} \\
    \mathsf{Equations} \{ \mathsf{Equations}( J ) \} \\
    \mathsf{Rewrites} \{ src ~> trgt \} \\
  \}
\end{align}
...
\begin{align}
  \mathsf{letrec} islands &= \\
  Locate( Pop, N, D ) &= \\
    \mathsf{foldLeft}( \\
      Pop, \\
      0, \\
      ( acc, e ) \mathsf{=>} \{ \\
        \mathsf{let} ( sex, chan ) \mathsf{=} ( \mathsf{rand}( 0, 1 ), \mathsf{choose}( N, D ) ) \mathsf{in} \\
        \mathsf{let} pheno = \\
          \mathsf{match} sex \mathsf{with} \{ \\
            \mathsf{case} 0 \mathsf{=>} \{ \\
              \mathsf{for}( t\mathsf{@}\mathsf{Theory}\{ \mathsf{terms}( e ), \mathsf{equations}( e ), r \} \mathsf{<-} chan )\{ \\
                Locate( e \otimes t, { chan }, \mathsf{Delta}( chan ) ) \\
              \} \\
            \}
            \mathsf{case} 1 \mathsf{=>} \{ chan\mathsf{!}( e ) \} \\
         \} \mathsf{in} \\
          acc \mathsf{|} pheno \\
      \} \\
    ) \mathsf{in} \\
   \mathsf{free}( \mathsf{MeTTaIL} ) \mathsf{=>} \{ \\
      islands \\
   \} 
\end{align}
