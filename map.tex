...
\section{Appendix C: Inference rules expected to be generated by the hypercube functor acting on the GSLT for RHO calculus}
\label{Appendix C}
...
In the rule below, the name $x$ has to be part of the conclusion because the left-hand side of the comm rule duplicates $x$.  It makes sense with either choice of $s_1$ or $s_2$: the former allows sending types at runtime, while the latter only allows sending types at compile time while doing type checking.
\begin{prooftree}
...
The functor adds an extra parameter to $\rm{for}$ to track the type of the bound variable.  Similar to the case above, the rule makes sense with either $s_1$ or $s_3$.  It has to be the same choice as the send in order for the par in the left-hand side of the comm rewrite to be well-typed.  Because of that, choosing $s_2$ is never consistent.
\begin{prooftree}
\AxiomC{$\Gamma \vdash A : s_1^N$}
\AxiomC{$\Gamma \vdash x : A$}
